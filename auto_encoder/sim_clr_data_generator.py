...
class SimCLRDataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, tags_temp):
        ...
        for i, tag in enumerate(tags_temp):
            ...
            img = cv2.resize(img, (int(self.image_size[1]), int(self.image_size[0])), interpolation=cv2.INTER_CUBIC)
            tar = cv2.resize(tar, (int(self.image_size[1]), int(self.image_size[0])), interpolation=cv2.INTER_CUBIC)
            x.append(img)
            y.append(tar)
